...
def process(source, dest):
    ...
    source_file = station_mapping[station_mapping['Station'] ==
                                  source]['Filename'].values[0]
    dest_file = station_mapping[station_mapping['Station'] ==
                                dest]['Filename'].values[0]
...
def get_route():
    ...
    if not source or not destination:
        return jsonify({'error':
                        'Please provide both source and destination'}), 400
